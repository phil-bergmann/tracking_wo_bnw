...
from tracker.simple_flow_tracker import Simple_Flow_Tracker
...
Simple_Flow_Tracker = ex.capture(Simple_Flow_Tracker, prefix='simple_tracker.tracker')
...
def my_main(simple_tracker, _config):
    ...
    print("[*] Building FlowNet")
    args = argparse.Namespace()
    args.rgb_max = 255.0
    args.fp16 = False
    args.cuda = True
    args.grads = {}
    flow = FlowNet2(args, False, 20.0)
    flow.eval()
    flow.cuda()
    flow.load_state_dict(torch.load('/usr/stud/bergmanp/FlowNet2_checkpoint.pth.tar')['state_dict'])
    ...
    if simple_tracker['mode'] == 'simple_flow':
        print("[*] Using Simple")
        tracker = Simple_Flow_Tracker(frcnn=frcnn, flownet=flow)
    ...
    for s in sequences:
        ...
        for i,sample in enumerate(dl,1):
            tracker.step(sample)
