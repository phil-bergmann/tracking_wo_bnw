...
from tracker.triplet_loss import batch_all_triplet_loss
from tracker.triplet_loss import batch_hard_triplet_loss
from tracker.triplet_loss import _pairwise_distances
from tracker.triplet_loss import _get_triplet_mask
from tracker.triplet_loss import _get_anchor_positive_triplet_mask
from tracker.triplet_loss import _get_anchor_negative_triplet_mask
...
def test_pairwise_distances():
	...
	for squared in [True, False]:
		...
		res_pt = _pairwise_distances(Variable(torch.from_numpy(embeddings)), squared=squared).data.numpy()
...
def test_gradients_pairwise_distances():
	...
	for squared in [True, False]:
		var = Variable(torch.from_numpy(embeddings), requires_grad=True)
		dists = _pairwise_distances(var, squared=squared)
		dists.mean().backward()

		g = var.grad.data.numpy()
...
def test_triplet_mask():
	...
	mask_pt = _get_triplet_mask(torch.from_numpy(labels)).numpy()

	assert np.allclose(mask_np, mask_pt)
...
def test_anchor_positive_triplet_mask():
	...
	mask_pt = _get_anchor_positive_triplet_mask(torch.from_numpy(labels)).numpy()
...
def test_anchor_negative_triplet_mask():
	...
	mask_pt = _get_anchor_negative_triplet_mask(torch.from_numpy(labels)).numpy()
...
def test_simple_batch_all_triplet_loss():
	...
	for squared in [True, False]:
		...
		loss_pt, fraction = batch_all_triplet_loss(torch.from_numpy(labels),
								Variable(torch.from_numpy(embeddings)), margin, squared=squared)
		loss_pt = loss_pt.data.numpy()
		fraction = fraction.data.numpy()

		assert np.allclose(loss_np, loss_pt)
		assert np.allclose(fraction, 0.0)
...
def test_batch_all_triplet_loss():
	...
	for squared in [True, False]:
		...
		assert np.allclose(loss_np, loss_pt)
		assert np.allclose(num_positives / num_valid, fraction)
...
def test_batch_hard_triplet_loss():
	...
	for squared in [True, False]:
		...
		# Compute the loss in PyTorch.
		loss_pt = batch_hard_triplet_loss(torch.from_numpy(labels),
								Variable(torch.from_numpy(embeddings)), margin, squared=squared)
		loss_pt = loss_pt.data.numpy()
