...
def extract_valid_gt_data(all_data, remove_ofv=False):
    ...
    selected = np.array([i for i in range(all_data.shape[0]) if all_data[i, 7] in valid_classes + distractor_classes])
...
def print_format(widths, formaters, values, form_attr):
    return ' '.join([(form_attr%(width, form)).format(val) for (form, width, val) in zip(formaters, widths, values)])

def print_format_name(widths, values, form_attr):
    return ' '.join([(form_attr%(width)).format(val) for (width, val) in zip(widths, values)])
...
def print_metrics_ext(header, metrics, banner=30):
    ...
    metric_widths_long = [5, 4, 4, 6, 9, 16, 9, 14, 17, 11, 15, 15, 11, 14, 5, 5, 8];
