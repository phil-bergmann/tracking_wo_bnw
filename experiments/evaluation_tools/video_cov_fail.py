...
def my_main(_config):
    ...
    module_dir = osp.join(module_dir, 'cov_fail')
    ...
    for t in tracker:
        ...
        for db in Datasets(dataset):
            ...
            for frame in range(f_gt):
                ...
                tracked_gt = []
                for gid in gids:
                    tracked_gt.append(gt_ids_res[gid])

                total_gt = list(gt_inds[frame].keys())

                for gt in total_gt:
                    if gt not in tracked_gt:
                        res = np.zeros(6)
                        res[0] = frame+1
