...
class Tracker():
	...
	def regress_tracks(self, blob, det_pos, det_scores):
		cl = 1
		...
		# get scores from detections with kuhn munkres #####
		#scores = scores[:,cl]
		scores = torch.zeros(pos.size(0)).cuda()

		if det_pos.nelement() > 0:
			# calculate IoU distances
			iou_inv = 1 - bbox_overlaps(pos, det_pos)
			iou_inv = iou_inv.cpu().numpy()
			...
			row_ind, col_ind = linear_sum_assignment(iou_inv)
			for r,c in zip(row_ind, col_ind):
				if iou_inv[r,c] <= 1 - self.detection_nms_thresh:
					#print("im: {}\ncosts: {}".format(self.im_index, costs[r,c]))
					scores[r] = det_scores[c]
		####################################################
	...
	def step(self, blob):
		...
		if self.public_detections:
			...
			if len(dets) > 0:
				dets_sc = torch.cat(dets, 0).cuda()
				sc = dets_sc[:,4]
				dets = dets_sc[:,:4]
				_, scores, bbox_pred, rois = self.frcnn.test_rois(dets)
				rois = dets
				scores = torch.stack([torch.zeros(sc.size()).cuda(),sc], 1)
				bbox_pred = torch.zeros((rois.size(0), 8)).cuda()
			else:
		...
		if len(self.tracks) > 0:
			...
			if self.do_align:
				self.align(blob)
			#regress
			person_scores = self.regress_tracks(blob, det_pos, det_scores)
