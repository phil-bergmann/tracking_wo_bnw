...
class Simple_Regressor_Tracker():

	def __init__(self, frcnn, simple_regressor, detection_person_thresh, regression_person_thresh, detection_nms_thresh,
		regression_nms_thresh, alive_patience):
		self.frcnn = frcnn
		self.simple_regressor = simple_regressor
	...
	def add(self, new_det_pos):
		...
		hidden, cell_state = self.simple_regressor.init_hidden(num_new)
	...
	def step(self, blob):
		...
		if self.pos.nelement() > 0:
			# regress
			_, _, _, rois = self.frcnn.test_rois(self.pos)
			search_features = Variable(self.frcnn.get_fc7())
			bbox_pred, (self.hidden, self.cell_state) = self.simple_regressor.forward(search_features, (self.hidden, self.cell_state))
			boxes = bbox_transform_inv(rois, bbox_pred.data)
			boxes = clip_boxes(Variable(boxes), blob['im_info'][0][:2]).data
			self.pos = boxes
