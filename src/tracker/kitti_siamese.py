...
from .kitti_sequence import KITTI_Sequence
...
class KITTI_Siamese(KITTI_Sequence):
	...
	def build_samples(self):
		...
		for sample in self.data:
			...
			for k,v in tracks.items():
				if k in gt.keys():
					v.append({'id':k, 'im_path':im_path, 'gt':gt[k], 'trunc':trunc[k], 'occl':occl[k]})
					del gt[k]

			# For all remaining BB in gt new tracks are created
			for k,v in gt.items():
				tracks[k] = [{'id':k, 'im_path':im_path, 'gt':v, 'trunc':trunc[k], 'occl':occl[k]}]
		...
		if self._seq_name:
			print("[*] Loaded {} objects from sequence {}.".format(len(res), self._seq_name))
