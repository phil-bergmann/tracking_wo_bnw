...
from mask_rcnn_tracktor import MaskRCNNTracktor
from keypoint_rcnn_tracktor import KeypointRCNNTracktor
...
def retinanet_resnet50_fpn(pretrained=False, progress=True,
    ...
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls['retinanet_resnet50_fpn_coco'],
                                              progress=progress)
        if num_classes == 2:
            print('Load person detection weights from COCO.')

            # idx = list(range(18))
            idx = [0, 1, 91, 92, 182, 183, 273, 274, 364, 365, 455, 456, 546, 547, 637, 638, 728, 789]
            state_dict['head.classification_head.cls_logits.weight'] = state_dict['head.classification_head.cls_logits.weight'][idx]
            state_dict['head.classification_head.cls_logits.bias'] = state_dict['head.classification_head.cls_logits.bias'][idx]
