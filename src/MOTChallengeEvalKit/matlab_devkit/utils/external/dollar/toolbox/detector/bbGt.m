...
function [xs,ys,score,ref,  tp_array, np] = compRoc( gt, dt, roc, ref )
% Compute ROC or PR based on outputs of evalRes on multiple images.
%
...
%
% USAGE
%  [xs,ys,score,ref, tp_array, np] = bbGt( 'compRoc', gt, dt, roc, ref )
%
% INPUTS
...
m=length(ref); np=size(gt,1); score=dt(:,5); tp_array=dt(:,6);

[score,order]=sort(score,'descend');
tp_array=tp_array(order);
fp=double(tp_array~=1);
fp=cumsum(fp); tp=cumsum(tp_array);
